 ...
 *   getPolynom()      => null
 */
function getPolynom(...c) {
  return (x) => c.reduceRight((a, k, i) => a + k * x ** (c.length - i - 1));
}
...
function memoize(func) {
  const argRes = new Map();

  return (...arg) => {
    const key = arg.length + arg.join('+');

    if (argRes.has(key)) {
      return argRes.get(key);
    }
...
function logger(func, logFunc) {
  return (...args) => {
    const signature = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
